		...
		chrome.tabs.executeScript(pandoraTabId, {
			...
			if (callback == "playing") {
				chrome.browserAction.setIcon({
					path : "action-pause.png"
				});
				isPlaying = true;
			} else if (callback == "paused") {
				chrome.browserAction.setIcon({
					path : "action-play.png"
				});
				isPlaying = false;
			}
		});
...
function checkIfPandoraHasScripts() {
	chrome.tabs.sendMessage(pandoraTabId, {
		greeting : "hello"
	}, function (response) {
		if (response) {
			if (DEBUG) {
			...
			chrome.tabs.executeScript(pandoraTabId, {
				file : "areyoulistening_loop.js"
			});
			chrome.tabs.executeScript(pandoraTabId, {
				code : "getPlaying(); getSongInfo();"
			});
		}
	});
}
...
function goToPandora() {
	if (alreadyClicked && pandoraTabId != null) {
		...
		// checks if it's pause or skip
		// gets to skip after double clicking the pauseNext button
		if (pauseNext) {
			skip = true;
			pauseNext = false;
		}
		if (skip||!parseBool(localStorage["togglePlayNext"])) {
			chrome.browserAction.setIcon({
				path : "action-pause.png"
			});
			chrome.tabs.executeScript(pandoraTabId, {
		...
		//triggers pausenext instead of skipping
		else if (parseBool(localStorage["togglePlayNext"])) {
			chrome.browserAction.setIcon({
				path : "action-pause-next.png"
			});
			pauseNext = true;
		}
		//Clear all Clicks
...
function onMessage(request, sender, sendResponse) {
	...
	case "paused":
		//checks which icon it should use
		if (pauseNext) {
			chrome.browserAction.setIcon({
				path : "action-play-next.png"
			});
		} else {
			chrome.browserAction.setIcon({
				path : "action-play.png"
	...
	case "songChanged":
		...
		if (parseBool(localStorage["showNotifications"])) {
			var options = {
				...
				items : [{
						title : "Artist",
						message : request.songArtist
					}, {
						title : "Album",
						message : request.songAlbum
					}
				]
			};
			chrome.notifications.create(options.title, options, function () {});
			setTimeout(function () {
				chrome.notifications.clear(options.title, function () {});
			}, 5000);
		}
		if (pauseNext) {
			pauseNext = false;
